...
class NovelFingerprintData:
    ...
    def __init__(self, mol_data, frag_data, group_data, chain_data):
        self.md, self.fd, self.gd, self.cd = mol_data, frag_data, group_data, chain_data
    ...
    def fingerprint_mol(self, mol_id):
        ...
        for idx, row in mg.iterrows():
            ...
            groups.add(group)
        mfp = NovelFingerprint(g_dict, s_dict, b_dict, groups, bridges, subs)
    ...
    def mols_same_group(self, counts=False, pattern="", exclude_benz=False):
        if pattern:
            mol_patt_ids = self.md.find_mols_with_pattern(
                pattern, clean_only=True
            ).index

            g_fps = self.get_group_fps(counts, exclude_benz=exclude_benz)
            mol_ids = list(set.intersection(set(mol_patt_ids), set(g_fps.index)))
            g_fps = g_fps[mol_ids]
        else:
            g_fps = self.get_group_fps(counts, exclude_benz=exclude_benz)
        g_fps = g_fps.apply(self._prep_fp_for_grouping)
        tmp_dict = g_fps.groupby(g_fps).groups
        groups_dict = {key: value for key, value in tmp_dict.items() if len(value) > 1}
        return groups_dict
    ...
    def mols_with_fp(self, fp, counts=True):
        ...
        if groups:
            s1 = set(
                self.group_fp_query(set(groups), complete=False, counts=counts).index
            )
            sets.append(s1)
        if bridges:
            s2 = set(
                self.bridge_fp_query(set(bridges), complete=False, counts=counts).index
            )
            sets.append(s2)
        if subs:
            s3 = set(self.sub_fp_query(set(subs), complete=False, counts=counts).index)
            sets.append(s3)
            ...
            for mol in mols:
                ...
                for mol_c in mols:
                    ...
                    if len(diff) == 1:
                        ...
                        total_dict[label].update([mol.get_id(), mol_c.get_id()])
        return total_dict
    ...
    def mols_diff_topology(self):
        ...
        top_dict = {key: value for key, value in tmp_dict.items() if len(value) > 1}
        return top_dict
    ...
    def same_fp_except_sub(self, sub_id):
        ...
        ret_dict = {
            fp: mol_ids
            for fp, mol_ids in tmp_dict.items()
...
class BasicFingerprint:
    ...
    def __init__(self, mol=None):
        self._fp_dict = {}
        self.fp_map = {
            "MACCS": self._calc_MACCS,
            "morgan": self._calc_morgan,
            "rdk": self._calc_rdk,
            "morgan_feat": self._calc_morgan_feat,
