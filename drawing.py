...
def draw_to_png_stream(mols, legends=[], to_disc=False):
    ...
    d2d = rdMolDraw2D.MolDraw2DCairo(
        full_size[0], full_size[1], sub_img_size[0], sub_img_size[1]
    )
...
def draw_entities(entities, outdir, from_idx, to_idx):
    ...
    freq_df = freq_df.sort_values(by="occurrence", ascending=False).iloc[
        from_idx:to_idx
    ]
    entities = freq_df["entity"]
    legends = freq_df.apply(
        lambda f: f"id: {f['entity'].get_id()}, freq: {f['occurrence']}", axis=1
    )
    draw_mols_canvas(entities, legends, outdir=outdir, start_idx=from_idx, per_row=5)
