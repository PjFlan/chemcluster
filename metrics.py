...
class Metric:
    ...
    def _int_to_cat(self, data, cut_off, bin_size=1):
        ...
        cat_data = data.apply(
            lambda x: self._assign_bin(x, bin_size, bins)
            if x < upper_bin
            else bins[upper_bin]
        )
    ...
    def _create_bins(self, size, upper_bin):
        bins = {
            i: ("-".join([str(i), str(i + size)]) if size > 1 else str(i))
    ...
    def categ_histogram(self, data, ax, cut_off=0.95, bin_size=1):
        ...
        if data.dtype.name != "category":
            categ_data, bins_order = self._int_to_cat(
                data, cut_off=cut_off, bin_size=bin_size
            )
        sns.countplot(
            categ_data, order=bins_order, palette=sns.color_palette("husl", 8), ax=ax
        )
        ax.set_ylabel("Occurrence")
        ax.text(0.8, 0.9, f"N = {data.size:,}", transform=ax.transAxes)
        return ax
...
class GroupMetric(Metric):
    ...
    def comp_data_report(self, id_):
        ...
        ret_string = (
            f"wavelength: {parent_lam.mean(): .5f} +- {parent_lam.std(): .3f}\n"
        )
        ret_string = (
            f"{ret_string}strength: {parent_f.mean(): .3f} +- {parent_f.std():.3f}"
        )
        print(ret_string)
    ...
    def group_fp_scatter(self, min_size=15, counts=True, save_as=None):
        ...
        for i in range(0, plots):
            ...
            to_idx = from_idx + 6
            tmp_df = comb_df[
                comb_df["Category"].gt(from_idx) & comb_df["Category"].le(to_idx)
            ]
...
class MoleculeMetric(Metric):
    def __init__(self, md, fd, gd, cd, nfp):
        ...
        self.dist_func_map = {
            "ac": self.arom_cycles,
            "ahc": self.arom_het_cycles,
            "ha": self.heteroatom_count,
            "hal": self.count_halogen,
            "cj": self.conjugation_count,
            "lfc": self.largest_frag_count,
        }
    ...
    def mult_dist_plot(
        self, shape, metrics=None, bin_sizes=[], painters=None, save_as=None
    ):
        # self.set_fonts()
    ...
    def dist_plot(
        self, metric, paint, bin_size=1, ax=None, is_mult=False, save_as=None
    ):
        # self.set_fonts()
        if not ax:
            ax = plt.gca()
    ...
    def conjugation_count(self):
        ...
        conj_bonds = self.md.get_molecules().apply(lambda x: x.get_conjugation())
    ...
    def comp_hexbin(self, save_as=None):
        ...
        ax = plt.gca()
        p = ax.hexbin(
            strength, lambdas, gridsize=25, cmap="summer", mincnt=1, norm=PowerNorm(0.4)
        )
