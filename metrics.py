...
class Metric:
    ...
    def draw_mols_canvas(self,mols,legends,outdir,suffix,start_idx=0,per_img=20,per_row=5):
        ...
        while file_num<=num_files:
            ...
            curr_mols = mols[file_begin:file_end]
            ...
            d2d.DrawMolecules(curr_mols,legends=legends[file_begin:file_end])
            d2d.FinishDrawing()
...
class FragmentMetric(Metric):
    ...
    def _fragment_freq(self,frags):
        freqs = frags.apply(lambda f: f.occurrence)
        cnt = Counter(freqs.to_dict())
        cnt = cnt.most_common()
        return list(zip(*cnt))
    
    def draw_frags(self,from_idx=0,to_idx=200):
        clean_frags = self.frag_data.clean_frags()
        ff = self._fragment_freq(clean_frags)
        
        ordered_smiles = ff[0]
        freqs = ff[1]
        ids = [self.mol_data.get_id_from_smiles(smi) for smi in ordered_smiles]
        frag_mols = clean_frags.apply(lambda frag: frag.get_rdk_mol())
        frag_mols = frag_mols.tolist()[from_idx:to_idx]
        legends = [f'id:{ids[i]}, freq:{freqs[i]}' for i in range(0,len(freqs))][from_idx:to_idx]
        frag_dir = os.path.join(self._config.get_directory('images'),'fragments')
        
        self.draw_mols_canvas(frag_mols,legends,outdir=frag_dir,suffix='',start_idx=from_idx,per_img=20,per_row=5)
          
    def similarity(self,ids):
        frags = self.frag_data.clean_frags()
        frag_smiles = map(self.frag_data.get_smiles_from_id,ids)
        frags = frags[frag_smiles]
        fps_1 = frags[0].fingerprint()
        fps_2 = frags[1].fingerprint()
...
class MoleculeMetric(Metric):
    ...
    def draw_parent_mols(self,group='',smi_id=None,from_idx=0,to_idx=200):
        self.mol_data.get_molecules()
        self.mol_data.set_comp_data()
        if not smi_id:
            parents = self.mol_data.find_mols_with_pattern(group).tolist()
        else:
            parents = self.mol_data.get_parent_mols(smi_id).tolist()
            group = str(smi_id)
        mols = [m.get_rdk_mol() for m in parents]
        legends = [f'amp: {m.lambda_max}nm f: {m.strength_max:.4f}' 
                    if m.lambda_max else '' for m in parents]
        ...
        if to_idx and to_idx < len(mols):
            mols = mols[from_idx:to_idx]
            legends = legends[from_idx:to_idx]
        suffix = group + '_'
        self.draw_mols_canvas(mols=mols,legends=legends,outdir=folder,start_idx=from_idx,suffix=suffix,
                              per_img=12,per_row=3)
        
    def mult_dist_plot(self,shape,metrics=[],painters=[],save_as=None):
