...
class MoleculeData(EntityData):
    ...
    def _comp_filter(self, mol, f_min, lambda_max, lambda_min):
        ...
        if "." in mol.smiles:
            return None
        if (
            lambda_min < mol.get_lambda_max() < lambda_max
    ...
    def set_conjugation(self, molecules=None):
        if not molecules:
            molecules = self.get_molecules()
        conj_file = os.path.join(
            self._config.get_directory("conjugation"), "conjugation.csv"
        )
    ...
    def find_mols_with_pattern(self, patt, clean_only=False):
        ...
        matches = mols.apply(lambda x: x if x.has_pattern(pattern) else None)
    ...
    def find_mols_no_pattern(self, clean_only=False):
        ...
        if clean_only:
            ...
            mols = self.get_molecules()
        patterns = [
            Chem.MolFromSmarts(smarts) for smarts in list(self.PATTERNS.values())
        ]
        tmp_mols = []
    ...
    def clean_mols(self):
        ...
        molecules = molecules.apply(
            self._comp_filter, args=(f_min, lambda_max, lambda_min)
        )
...
class GroupData(EntityData):
    ...
    def _first_round_clustering(self, groups, diverse_groups, clusters):
        ...
        for group in diverse_groups:
            ...
            clust_groups = tuple(remaining_groups[sim_groups.index].index) + (
                group.get_id(),
            )
    ...
    def _get_direct_subs(self):
        ...
        direct_subs = {sub: self._direct_sub_pattern(sub) for sub in direct_subs}
        return direct_subs
    ...
    def _create_taa_group(self, groups_dict, mol_group_link, id_):
        ...
        new_link = [
            [group_mol_id, taa_group.get_id()] for group_mol_id in group_mol_ids
        ]
    ...
    def _basic_clustering(self):
        ...
        clusters = []
        remaining_groups = self._first_round_clustering(
            groups, diverse_groups, clusters
        )
    ...
    def get_groups(self, regen=False):
        ...
        for frag in frags:
            ...
            new_link = [
                [group_mol_id, group.get_id()] for group_mol_id in group_mol_ids
            ]
            mol_group_link.extend(new_link)
...
class ChainData(EntityData):
    ...
    def _get_chain_df(self, reset=False):
        ...
        for mol in mols:
            ...
            matches = [
                self._get_matches(mol.get_rdk_mol(), g.get_rdk_mol(), other_atoms)
                for g in groups
            ]
            records = [
                [mol.get_id(), mol, g, g.get_rdk_mol(), matches[i], other_atoms[i]]
    ...
    def _get_bridges(self, my_mol, rdk_groups, mol_subs, bridges_dict):
        ...
        for rdk_group in rdk_groups[::-1]:
            while trimmed_mol:
                ...
                trimmed_mol = Chem.ReplaceCore(
                    trimmed_mol, rdk_group, labelByIndex=True
                )
            trimmed_mol = Chem.MolFromSmiles(tmp)
        smiles = Chem.MolToSmiles(trimmed_mol)
        branches = smiles.split(".")
        branches = [re.sub("\[*[0-9]*\*+\]*", "[*]", branch) for branch in branches]
        branches = [
            branch
            for branch in branches
            if (not re.match("^(\[*\*\]*)+$", branch)) and (branch != "")
        ]
        for sub in mol_subs:
            if sub in branches:
                branches.remove(sub)
    ...
    def _get_substituents(
        ...
        for i in range(0, len(matches)):
            ...
            new_records = [
                [my_mol.get_id(), my_group.get_id(), i, sub.get_id()]
                for sub in obj_subs
            ]
    ...
    def get_substituents(self, regen=False):
        ...
        for idx, row in df.iterrows():
            ...
            self._get_substituents(
                my_mol, group, rdk_groups, other_atoms, matches, mol_subs_map, subs_dict
            )
