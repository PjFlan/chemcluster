...
class Molecule(Entity):
    ...
    def fragment(self):
        ...
        leaf_frags = []
        if self.get_rdk_mol():
            leaf_frags = BRICS.BRICSDecompose(
                self.get_rdk_mol(), keepNonLeafNodes=False
            )
        return leaf_frags
    ...
    def get_comp_data(self):
        try:
            comp_dict = {"lambdas": self._lambdas, "strengths": self._strengths}
            return comp_dict
        except AttributeError:
    ...
    def get_legend(self, dev_flag=True):
        ...
        if dev_flag:
            return (
                f"{self.get_id()} ; {self._lam_max} nm ; {self._osc_max:.4f}"
                if self._lam_max
                else ""
            )

        return (
            f"{self.lambda_max} nm ; {self.strength_max:.4f}" if self.lambda_max else ""
        )
